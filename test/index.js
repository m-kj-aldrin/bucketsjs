...
const rand = (min = -1, max = 1) => min + Math.random() * (Math.abs(min) + Math.abs(max));
...
const randChoice = /**@template {any[]} T @param {T} arr */ (arr) => arr[Math.floor(Math.random() * arr.length)];

function updateFront() {
  train[0].x.setTarget(target.x, 300);
  train[0].y.setTarget(target.y, 300);
...
function draw() {
  // console.log({ dx, dy });
  //   target.x += dx * 1 * Math.random() * 1;
  //   target.y += dy * 1 * Math.random() * 1;
